...
def val(labels_and_preds):
    acc = 0
    all = 0
    for each in labels_and_preds.filter(lambda x: x[1] > 0.80).collect():
        if each[0] == 1.0:
            acc += 1
        all += 1
    ...
    acc = 0
    all = 0
    for each in labels_and_preds.filter(lambda x: x[1] < 0.20).collect():
        if each[0] == 0.0:
            acc += 1
        all += 1
    negtive35_acc = acc * 1.0 / all
    print "negtive35:", acc, all, negtive35_acc
...
def main(sc, sql_context, is_hive = True):
    ...
    f_check = get_labeled_points("2015-01-01", "9999-99-99",sc, sql_context, is_hive)
