...
def cal_feature_per(x, window, coach):
    ...
    for i in range(0, len(x) - window):
        ...
        for j in range(1, window+1):
            ...
            l_feature.append(x[i+j].close / x[i].close)
        ...
        if i+window+coach >= len(x):
            ...
            date3 = ""
            is_labeled = 0
        elif x[i+window+coach].close / x[i+window].close >= 1.02 :
            cls = 1
...
def get_lp(sc, sql_context, is_hive):
    ...
    df_lp = df_close.join(df_mat, [df_close.symbol == df_mat.symbol, df_close.date == df_mat.date], 'inner' )\
            .select(df_close.symbol.alias("symbol"), df_close.date.alias("date"), df_close.close.alias("close"), df_mat.mat_dual.alias("mat_dual"))
...
def save(lp, sql_context, is_hive):
    ...
    if not is_hive:
        df.registerAsTable("label_point_pos")
        return

    sql_context.sql("""
        CREATE TABLE IF NOT EXISTS label_point_pos(
            symbol string,
            act_diff float,
            is_labeled int,
            date1 string,
    ...
    df.repartition(64).insertInto("label_point_pos", overwrite = True)
